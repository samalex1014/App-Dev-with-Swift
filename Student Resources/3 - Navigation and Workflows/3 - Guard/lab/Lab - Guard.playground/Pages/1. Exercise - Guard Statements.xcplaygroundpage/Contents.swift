...
print(calculateArea(width: 30, height: 2))
print(calculateArea(width: -1, height: -3))
/*:
...
func sum(one: Int?, two: Int?) -> Int? {
    guard let first: Int = one, let second: Int = two else {
        return nil
    }
    return first + second
}

print(sum(one: 3, two: 2))
print(sum(one: 2, two: nil))
...
func createUser() -> User? {
    guard let first = firstNameTextField.text, let last = lastNameTextField.text, let age = ageTextField.text else {
        return nil
    }
...
if let unwrapped = user {
    print(unwrapped.firstName)
    print(unwrapped.lastName)
    print(unwrapped.age)
}
